...
assert.match(dashJs, /Zile libere și ore speciale/);
assert.match(dashJs, /zz\.ll\.aaaa/);
assert.match(dashJs, /Pauză după \(min\)/);
assert.match(dashJs, /Intervalul este din nou liber/);
assert.doesNotMatch(dashJs, /\bSlotul\b/);
assert.doesNotMatch(dashJs, /Timezone:|Blackout \/|Buffer \(min\)|mm\/dd\/yyyy/);
