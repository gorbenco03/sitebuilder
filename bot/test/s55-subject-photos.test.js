...
const PARENT_SHA = 'b32812274b7090ad370a1e8b2b74261a532ea6c1';
...
/** Read blob at parent commit for templates/<system>/images/<file>. */
function parentBlob(systemId, fileName) {
  const rel = `templates/${systemId}/images/${fileName}`;
  try {
    return execFileSync('git', ['-C', ROOT, 'show', `${PARENT_SHA}:${rel}`], {
      maxBuffer: 20 * 1024 * 1024,
    });
  } catch (e) {
    throw new Error(`parent blob missing ${rel}: ${e.message}`);
  }
}
...
check('every referenced image is ≥960×640 and ≥120 KiB', () => {
  for (const id of SYSTEMS) {
    ...
    for (const rel of refs) {
      ...
      assert.ok(
        st.size >= MIN_BYTES,
        `${id}/${rel}: size ${st.size} < ${MIN_BYTES} (still S54 texture scale?)`
      );
      const { w, h } = imageDimensions(abs);
      assert.ok(w >= MIN_W && h >= MIN_H, `${id}/${rel}: dims ${w}×${h} < ${MIN_W}×${MIN_H}`);
    }
  }
});

check('no leftover small files in three images/ dirs (all ≥960×640 and ≥120 KiB)', () => {
  for (const id of SYSTEMS) {
    ...
    for (const abs of files) {
      ...
      assert.ok(
        w >= MIN_W && h >= MIN_H,
        `${id}/${rel}: leftover small/abstract-scale image ${w}×${h}`
      );
    }
  }
});
...
check('every image file SHA-256 differs from parent S54 commit (no kept texture blobs)', () => {
  let compared = 0;
  for (const id of SYSTEMS) {
    const files = listImageFiles(id);
    for (const abs of files) {
      ...
      const parentBuf = parentBlob(id, fileName);
      const parentHash = sha256Buf(parentBuf);
      assert.notStrictEqual(
        cur,
        parentHash,
        `${id}/images/${fileName}: SHA still equals parent ${PARENT_SHA.slice(0, 12)} (S54 texture kept)`
      );
      // parent files are known abstract textures in the 45–86 KiB band
      assert.ok(
        parentBuf.length < MIN_BYTES,
        `${id}/images/${fileName}: unexpected parent size ${parentBuf.length} (test assumes S54 textures < 120KiB)`
      );
      compared += 1;
    }
  }
...
if (failed) {
  console.error('\ns55-subject-photos.test.js: FAILED');
  process.exit(1);
}
console.log('\ns55-subject-photos.test.js: all passed');
