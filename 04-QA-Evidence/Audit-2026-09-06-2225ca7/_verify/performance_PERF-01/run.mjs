...
const templateIds = ['product-menu', 'local-service'];
const results = {};
for (const tid of templateIds) {
  ...
  // Accept cookie banner if present (avoid it interfering, mirrors real user flow)
  try {
  ...
  const hasApiTemplates = responses.some(r => r.url.includes('/api/templates'));
  results[tid] = {
    ...
    totalBytesSinceClick: totalBytes,
    hasApiTemplatesCallSinceClick: hasApiTemplates,
    allResponses: responses,
  };
  console.log(tid, 'frameLoad=', tFrameLoad, 'requests=', responses.length, 'hasApiTemplates=', hasApiTemplates);
  await b.close();
}

fs.writeFileSync('/Users/Work/Desktop/sitebuilder/04-QA-Evidence/Audit-2026-09-06-2225ca7/_verify/performance_PERF-01/repro-detailed.json', JSON.stringify(results, null, 2));
await srv.close();
console.log('DONE');
