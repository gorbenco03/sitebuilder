...
// renderHtml() pipeline and served with the REAL cookie-banner.css/js
// (generated by bot/site-legal.js, exactly as a real export/publish would
// ship them) so the full first-time-visitor experience is reproduced.
// Usage: node measure-prof-cls-full.mjs <label> <templateHtmlPath> <stylesCssPath> <scriptJsPath> <outJsonPath>
...
async function main() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'prof-cls-full-' + label + '-'));
