...
**Hidook Site Builder** ships five **design systems** under `templates/` (`product-menu`, `local-service`, `portfolio`, `professionals`, `desserdirina`). Each folder is a vertical template: HTML/CSS/JS plus `schema.json` / `presets.json` for the wizard. The professionals system includes a local appointment-*request* flow (no external calendar). `desserdirina` is the commercial remake of the root bakery sample (cofetărie / cake shop).
