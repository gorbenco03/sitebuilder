...
// ─── Zero-dependency, conservative minifiers (PERF-04) ─────────────────────
//
// No npm package added (esbuild/terser would need network install and this
// product ships zero-dependency by design). A hand-rolled *general* JS
// minifier is unsafe without a real parser (regex literals, template
// strings and ASI hazards are easy to corrupt), so we deliberately do NOT
// minify JS bodies here — only whitespace/blank-line cleanup, which can
// never change token content. For CSS we can safely go further because the
// grammar is simple enough to protect string literals and strip comments
// without touching selectors or values (calc() spacing is preserved since
// we only collapse *runs* of whitespace into one space, never remove the
// single space that calc()/selector-combinators require).

const { minifyCss } = require('../bot/css-minify.js');

function trimJsWhitespace(js) {
    if (!js) return js;
    return js
        .split('\n')
...
/**
 * Insert the snippet immediately before the LAST body-closing tag.
 *
 * Both preview injections below used a string replace on the closing tag, and
 * String.replace with a string pattern rewrites the FIRST match. A template
 * that merely MENTIONS the literal </body> earlier in the document -- in an
 * HTML comment, say -- therefore captured the injection: the edit overlay was
 * spliced inside that comment and never executed, so the whole inline editor
 * silently died on that template with no error anywhere. That is exactly what
 * happened to templates/professionals when a comment explaining a fix quoted
 * the tag it was talking about.
 *
 * The legal-docs injection further down already worked around this with
 * lastIndexOf; this makes every injection agree.
 */
function insertBeforeBodyClose(html, snippet) {
    const tag = '</bod' + 'y>';
    let at = html.lastIndexOf(tag);
    if (at === -1) at = html.toLowerCase().lastIndexOf(tag);
...
function renderPreview(files, config, opts) {
    ...
    if (opts && opts.editMode) {
        ...
        html = insertBeforeBodyClose(html,
            '<script data-hidook-edit-overlay>' + EDIT_OVERLAY_SRC + '</script>'
        );
    }
...
const engineIifeOut = trimJsWhitespace(engineIife);
fs.writeFileSync(path.join(GEN_DIR, 'engine.js'), engineIifeOut, 'utf8');
console.log('  engine.js written (' + engineIifeOut.length + ' bytes)');
...
for (const entry of registry.templates) {
    ...
    const files = {
        // templateHtml is left byte-for-byte as authored: it carries the
        // <!-- @if/@each/@end --> templating directives that build.js's
        // renderHtml() parses as control structures, and HTML minification
        // (safely) needs a real parser to avoid corrupting those or
        // whitespace-sensitive inline content.
        templateHtml: fs.readFileSync(path.join(dir, 'template.html'), 'utf8'),
        // CSS minification is safe here (comments + whitespace only, see
        // minifyCss above) and shrinks the per-template heavy JS payload
        // that ships on every "Start" click (PERF-01/PERF-04).
        stylesCss:    minifyCss(fs.readFileSync(path.join(dir, 'styles.css'), 'utf8')),
        // JS minification is intentionally NOT attempted beyond whitespace
        // cleanup — see the comment above trimJsWhitespace().
        scriptJs:     trimJsWhitespace(fs.readFileSync(path.join(dir, 'script.js'), 'utf8')),
    };


    const collageFile = path.join(dir, 'collage.js');
    if (fs.existsSync(collageFile)) {
        files.collageJs = trimJsWhitespace(fs.readFileSync(collageFile, 'utf8'));
    }
